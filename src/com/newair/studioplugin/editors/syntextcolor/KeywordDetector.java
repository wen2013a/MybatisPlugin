...
public class KeywordDetector implements IWordDetector {
	...
	public boolean isWordStart(char c) {
		// 循环所有的关键字,如果有关键字中的第一个字符匹配该字符，则返回true
		char cc = Character.toUpperCase(c);
		for (int i = 0, n = TextConst.SQL_KEY_WORD.length; i < n; i++) {
			if (cc == ((String)TextConst.SQL_KEY_WORD[i]).charAt(0))
				return true;
		}
		return false;
	}
	...
	public boolean isWordPart(char c) {
		// 循环所有的关键字,如果关键字的字符中有该字符，则返回true
		char cc = Character.toUpperCase(c);
		for (int i = 0, n = TextConst.SQL_KEY_WORD.length; i < n; i++) {
			if (((String)TextConst.SQL_KEY_WORD[i]).indexOf(cc) != -1)
				return true;
		}
		return false;
	}
